...
class PhraseTrigger(Trigger):
    ...
    def in_phrase_in(self, text):
        ...
        for punc in string.punctuation:
            if punc in text.lower():
                text_without_punc = text_without_punc.replace(punc, ' ')
        
        
        word_list = text_without_punc.lower().split(" ")
        new_word_list = []        
        ...
        phrase_list = self.get_phrase().lower().split(" ")
        word_index = 0
        count = 0
        word_count = 0
        for word in new_word_list:
            if word == phrase_list[0]:
                ...
                while count < len(phrase_list):
                    if (count + 1) > len(phrase_list) or word_index >= len(new_word_list):
                        break
                    else:
                        if phrase_list[count] == new_word_list[word_index]:
                            word_index += 1
                            word_count += 1
                        else:
                            break
...
class BeforeTrigger(TimeTrigger):
    ...
    def evaluate(self, story):       
        story_datetime =  story.get_pubdate().replace(tzinfo=pytz.timezone("EST"))
        if story_datetime < self.time:
            return True
        else:
            return False
...
def read_trigger_config(filename):
    ...
    for line in lines:
        ...
        if temp_list[0] == "ADD":
            for name, trigger in trigger_dict.items():
                if trigger != None:
            ...
            if temp_list[1] == "TITLE":                
                ...
                trigger = trigger_dict.get(temp_list[2], None)
                trigger_dict[temp_list[0]] = NotTrigger(trigger)                

            elif temp_list[1] == "AND":
                trigger1 = trigger_dict.get(temp_list[2], None)
                trigger2 = trigger_dict.get(temp_list[3], None)
                trigger_dict[temp_list[0]] = AndTrigger(trigger1, trigger2)            
                

            elif temp_list[1] == "OR":
                trigger1 = trigger_dict.get(temp_list[2], None)
                trigger2 = trigger_dict.get(temp_list[3], None)
                trigger_dict[temp_list[0]] = OrTrigger(trigger1, trigger2)                    
...
def main_thread(master):
    ...
    try:
        t1 = TitleTrigger("murder")
        t2 = DescriptionTrigger("poisoning")
